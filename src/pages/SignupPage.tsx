...
const SignupPage: React.FC = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/v1/users/register', { name, email, password });
              ...
              <form className="space-y-6" onSubmit={handleSignup}>
                ...
                <div className="space-y-2">
                  <label htmlFor="name" className="text-sm font-medium text-foreground flex items-center gap-2">
                    <CheckCircleIcon className="h-4 w-4 text-primary" /> Name
                  </label>
                  <Input
                    id="name"
                    name="name"
                ...
                <div className="space-y-2">
                  <label htmlFor="confirm-password" className="text-sm font-medium text-foreground flex items-center gap-2">
                    <LockIcon className="h-4 w-4 text-primary" /> Confirm Password
                  </label>
                  <Input
                    id="confirm-password"
